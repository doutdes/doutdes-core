...
exports.ga_getMostPagesViews = async function (req, res, next) {
    GaToken.findOne({
        ...
        GoogleApi.getMostPagesVisited(key.client_email, key.private_key)
            .then(value => {
                if (value.length === 0) {
                    return res.status(HttpStatus.NO_CONTENT).send({});
                } else
...
exports.ga_getSources = async function (req, res, next) {
    GaToken.findOne({
        ...
        GoogleApi.getSources(key.client_email, key.private_key)
            .then(value => {
                if (value.length === 0) {
                    return res.status(HttpStatus.NO_CONTENT).send({});
                } else
...
exports.ga_getPageViewsByCountry = async function (req, res, next) {
    GaToken.findOne({
        ...
        GoogleApi.getPageViewsByCountry(key.client_email, key.private_key)
            .then(value => {
                if (value.length === 0) {
                    return res.status(HttpStatus.NO_CONTENT).send({});
                } else
