...
const setMetric = (metric, period, interval) => {
...
const ig_getData = async (req, res) => {
    ...
    try {
        key = await FbToken.findOne({where: {user_id: req.user.id}});
        data = await InstagramApi.getInstagramData(req.params.page_id, req.metric, req.period, req.since, req.until, key.api_key);
        return res.status(HttpStatus.OK).send(data);
    } catch (err) {
        console.error(err);
...
/** EXPORTS **/
module.exports = {setMetric, ig_getData, ig_getPages, ig_getMedia, ig_getVideos, ig_getImages};